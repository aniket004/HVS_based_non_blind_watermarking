...
str = int2str(ss);
s2=strcat(str,'.tif');
fileID = fopen('cropWATERLOO.txt','a+');
s1='F:\arpan\aniket\colorcomparison_31_08_15\matlab code\matlab code\image\WATERLOO\';
str=strcat(s1,s2)
fprintf(fileID,'%s ',s2);
...
%loading watermark
o=imread('F:\arpan\aniket\colorcomparison_31_08_15\matlab code\matlab code\image\logo\cameraman.jpg');
o=rgb2gray(o);
o=imresize(o,[64,64]);
level=graythresh(o);
o=im2bw(o,level);
...
fprintf(fileID,'%.4f ',PSNR);

% cropping

y = imcrop(y,[0,0,a/2,b/2]);
%temp = zeros(a,b,3);
%temp(1:a/2,1:b/2,1) = y1(:,:,1);
%temp(1:a/2,1:b/2,2) = y1(:,:,2);
%temp(1:a/2,1:b/2,3) = y1(:,:,3);
%y = y - temp;
%y1 = imcrop(y);
