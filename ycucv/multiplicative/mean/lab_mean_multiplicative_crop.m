...
[L_img, A_img, B_img] = RGB2Lab(R,G,B);


[a,b]=size(B_img);
I=imresize(B_img,[1024,1024]);
...
alpha = abs(mean(mean( jnd /det(u_s * v_s') ))/s_w(1,1));
...
%embedded watermark
B_img =imresize(I,[a,b]);
  
lab_image = cat(3,L_img,A_img,B_img);
...
s_new = abs(s1 - s_s)/alpha;

extracted_watermark = uu * s_new * vv' ;
